...
TagIpt.prototype.init=function() {
    //绑定事件
    addEvnetHandler(this.box, 'click', this.deleteEvent.bind(this));//删除元素事件的绑定
    addEvnetHandler(this.ipt, 'keyup', this.keyUp.bind(this));    //输入框输入内容事件的绑定
    addEvnetHandler(this.ipt, 'keydown', this.preventDefault);    //阻止输入框的默认事件
};
